...
totalCalories = fold1 . foodCalories

winner :: NonEmpty Elf -> Elf
winner = maximumBy (comparing totalCalories)

winners :: Int -> NonEmpty Elf -> NonEmpty Elf
winners n = takeN n . N.sortOn (Down . totalCalories)
 where
  takeN m = (N.:|) <$> N.head <*> ((take (m - 1)) . N.tail)

solve1 :: NonEmpty Elf -> Calories
solve1 = totalCalories . winner

solve2 :: NonEmpty Elf -> Calories
solve2 = foldMap1 totalCalories . winners 3
